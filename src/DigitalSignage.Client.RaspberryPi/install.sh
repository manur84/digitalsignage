...
if [ -d "$INSTALL_DIR" ]; then
    echo -e "${GREEN}Ã¢Å“â€œ${NC} Installation directory found: $INSTALL_DIR"

    if [ -d "$INSTALL_DIR/.git" ]; then
        GIT_REPO_EXISTS=true
        echo -e "${GREEN}Ã¢Å“â€œ${NC} Git repository exists"
    fi

    if [ -d "$VENV_DIR" ]; then
        VENV_EXISTS=true
        echo -e "${GREEN}Ã¢Å“â€œ${NC} Virtual environment exists"
    fi

    if [ -f "$INSTALL_DIR/config.py" ]; then
        CONFIG_EXISTS=true
        echo -e "${GREEN}Ã¢Å“â€œ${NC} Configuration file exists"
    fi
else
    echo -e "${YELLOW}Ã¢Å“â€”${NC} No installation directory found"
fi

if [ -f "$SERVICE_FILE" ]; then
    SERVICE_INSTALLED=true
    echo -e "${GREEN}Ã¢Å“â€œ${NC} Service installed"

    if systemctl is-active --quiet $SERVICE_NAME; then
        echo -e "${GREEN}Ã¢Å“â€œ${NC} Service running"
    else
        echo -e "${YELLOW}Ã¢Å¡Â ${NC} Service not running"
    fi
else
    echo -e "${YELLOW}Ã¢Å“â€”${NC} Service not installed"
fi
...
if [ -d "$INSTALL_DIR" ] && [ "$SERVICE_INSTALLED" = true ]; then
    MODE="UPDATE"
    echo ""
    echo -e "${BLUE}Mode: Ã°Å¸â€â€ž UPDATE${NC}"
else
    MODE="INSTALL"
    echo ""
    echo -e "${BLUE}Mode: Ã°Å¸â€œÂ¦ INSTALL${NC}"
fi

# Force fresh install in non-interactive mode (disabled; keep existing mode)
...
function show_warning() {
    echo -e "${YELLOW}Ã¢Å¡Â  $1${NC}"
}

function show_info() {
    echo -e "${BLUE}Ã¢â€žÂ¹ $1${NC}"
}
...
if [ "$MODE" = "UPDATE" ]; then
    ...
    for file in "${REQUIRED_FILES[@]}"; do
        if [ -f "$SCRIPT_DIR/$file" ]; then
            if cp "$SCRIPT_DIR/$file" "$INSTALL_DIR/" 2>/dev/null; then
                ...
                echo -e "  ${YELLOW}Ã¢Å¡Â  Failed to copy: $file${NC}"
            fi
        else
            echo -e "  ${RED}Ã¢Å“â€” Missing: $file${NC}"
            MISSING_FILES+=("$file")
        fi
    done
    ...
    if [ ${#MISSING_FILES[@]} -gt 0 ]; then
        echo ""
        echo -e "${YELLOW}Ã¢Å¡Â  Warning: Some files were not found in source:${NC}"
        ...
        if [ "$CRITICAL_MISSING" = true ]; then
            echo -e "${RED}Ã¢Å“â€” Critical files missing - cannot continue!${NC}"
            echo ""
            echo -e "${BLUE}Ã°Å¸â€™Â¡ TROUBLESHOOTING:${NC}"
            ...
            echo -e "${YELLOW}Ã¢Å¡Â  Non-critical files missing, continuing...${NC}"
        fi
    fi
...
if python3 -c "import PyQt5" 2>/dev/null; then
    ...
    echo -e "${RED}Ã¢Å“â€” PyQt5 installation failed${NC}"
    exit 1
fi
...
if [ ${#VERIFY_MISSING[@]} -gt 0 ]; then
    echo -e "${RED}Ã¢Å“â€” Verification failed, missing: ${VERIFY_MISSING[*]}${NC}"
    exit 1
fi
...
if timeout 15 sudo -u "$ACTUAL_USER" "$INSTALL_DIR/start-with-display.sh" --test; then
    ...
    if [ $TEST_EXIT_CODE -eq 124 ]; then
        echo -e "${RED}Ã¢Å“â€” Pre-flight check timed out${NC}"
    else
        echo -e "${RED}Ã¢Å“â€” Pre-flight check failed (exit code: $TEST_EXIT_CODE)${NC}"
    fi
    echo ""
