...
public class SystemDiagnosticsService
{
    ...
    public string ExportToText(SystemDiagnosticsReport report)
    {
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Status: {report.DatabaseHealth.Status}");
        sb.AppendLine($"Can Connect: {report.DatabaseHealth.CanConnect}");
        sb.AppendLine($"Provider: {report.DatabaseHealth.ProviderName}");
        sb.AppendLine($"Path: {report.DatabaseHealth.DatabasePath ?? "N/A"}");
        sb.AppendLine($"Size: {report.DatabaseHealth.DatabaseSize / 1024.0:F2} KB");
        if (report.DatabaseHealth.TableCounts.Any())
        {
            sb.AppendLine("Table Counts:");
            foreach (var kvp in report.DatabaseHealth.TableCounts)
            {
                sb.AppendLine($"  • {kvp.Key}: {kvp.Value}");
            }
        }
        if (report.DatabaseHealth.LastBackupDate.HasValue)
        {
            sb.AppendLine($"Last Backup: {report.DatabaseHealth.LastBackupDate:yyyy-MM-dd HH:mm:ss}");
        }
        sb.AppendLine();
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Status: {report.WebSocketHealth.Status}");
        sb.AppendLine($"Running: {report.WebSocketHealth.IsRunning}");
        sb.AppendLine($"Listening URL: {report.WebSocketHealth.ListeningUrl}");
        sb.AppendLine($"SSL/TLS: {(report.WebSocketHealth.EnableSsl ? "Enabled" : "Disabled")}");
        sb.AppendLine($"Active Connections: {report.WebSocketHealth.ActiveConnections}");
        sb.AppendLine($"Uptime: {report.WebSocketHealth.Uptime:dd\\.hh\\:mm\\:ss}");
        sb.AppendLine();
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Status: {report.PortAvailability.Status}");
        sb.AppendLine($"Configured Port: {report.PortAvailability.ConfiguredPort}");
        sb.AppendLine($"Port Available: {report.PortAvailability.IsConfiguredPortAvailable}");
        sb.AppendLine($"Current Active Port: {report.PortAvailability.CurrentActivePort}");
        if (report.PortAvailability.AvailablePorts.Any())
        {
            sb.AppendLine($"Available Alternative Ports: {string.Join(", ", report.PortAvailability.AvailablePorts)}");
        }
        sb.AppendLine();
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Status: {report.CertificateStatus.Status}");
        sb.AppendLine($"SSL Enabled: {report.CertificateStatus.SslEnabled}");
        if (report.CertificateStatus.SslEnabled)
        {
            sb.AppendLine($"Path: {report.CertificateStatus.CertificatePath ?? "N/A"}");
            sb.AppendLine($"Subject: {report.CertificateStatus.Subject ?? "N/A"}");
            sb.AppendLine($"Issuer: {report.CertificateStatus.Issuer ?? "N/A"}");
            if (report.CertificateStatus.ExpirationDate.HasValue)
            {
                sb.AppendLine($"Expires: {report.CertificateStatus.ExpirationDate:yyyy-MM-dd}");
            }
            sb.AppendLine($"Valid: {report.CertificateStatus.IsValid}");
        }
        sb.AppendLine();
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Status: {report.ClientStatistics.Status}");
        sb.AppendLine($"Total Clients: {report.ClientStatistics.TotalClients}");
        sb.AppendLine($"Online: {report.ClientStatistics.OnlineClients}");
        sb.AppendLine($"Offline: {report.ClientStatistics.OfflineClients}");
        sb.AppendLine($"Disconnected: {report.ClientStatistics.DisconnectedClients}");
        sb.AppendLine();
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Status: {report.PerformanceMetrics.Status}");
        sb.AppendLine($"CPU Usage: {report.PerformanceMetrics.CpuUsage:F2}%");
        sb.AppendLine($"Memory Usage: {report.PerformanceMetrics.MemoryUsageMB:F2} MB");
        sb.AppendLine($"Private Memory: {report.PerformanceMetrics.PrivateMemoryMB:F2} MB");
        sb.AppendLine($"Thread Count: {report.PerformanceMetrics.ThreadCount}");
        sb.AppendLine($"Disk Usage: {report.PerformanceMetrics.DiskUsagePercent:F2}% ({report.PerformanceMetrics.DiskFreeGB:F2} GB free)");
        sb.AppendLine();
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Status: {report.LogAnalysis.Status}");
        sb.AppendLine($"Log Files: {report.LogAnalysis.LogFilesCount}");
        sb.AppendLine($"Total Size: {report.LogAnalysis.TotalLogSizeMB:F2} MB");
        sb.AppendLine($"Errors (Last Hour): {report.LogAnalysis.ErrorsLastHour}");
        sb.AppendLine($"Warnings (Last Hour): {report.LogAnalysis.WarningsLastHour}");
        sb.AppendLine($"Errors (Today): {report.LogAnalysis.ErrorsToday}");
        sb.AppendLine($"Warnings (Today): {report.LogAnalysis.WarningsToday}");
        if (!string.IsNullOrWhiteSpace(report.LogAnalysis.LastCriticalError))
        {
            sb.AppendLine($"Last Critical Error: {report.LogAnalysis.LastCriticalError}");
        }
        sb.AppendLine();
        ...
        sb.AppendLine("───────────────────────────────────────────────────────");
        sb.AppendLine($"Machine Name: {report.SystemInfo.MachineName}");
        sb.AppendLine($"OS: {report.SystemInfo.OperatingSystem}");
        sb.AppendLine($"Processors: {report.SystemInfo.ProcessorCount}");
        sb.AppendLine($".NET Version: {report.SystemInfo.DotNetVersion}");
        sb.AppendLine($"Application Version: {report.SystemInfo.ApplicationVersion}");
        sb.AppendLine($"64-bit OS: {report.SystemInfo.Is64BitOperatingSystem}");
        sb.AppendLine($"64-bit Process: {report.SystemInfo.Is64BitProcess}");
        sb.AppendLine();
