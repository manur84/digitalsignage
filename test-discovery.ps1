...
if ($udpConnections) {
    Write-Host "SUCCESS: UDP Port 5555 is LISTENING" -ForegroundColor Green
    $processId = $udpConnections[0].OwningProcess
    Write-Host "  Process ID: $processId" -ForegroundColor Gray
} else {
    Write-Host "FAILED: UDP Port 5555 is NOT listening" -ForegroundColor Red
...
if ($tcpConnections) {
    Write-Host "SUCCESS: TCP Port 8080 is LISTENING" -ForegroundColor Green
    $processId = $tcpConnections[0].OwningProcess
    Write-Host "  Process ID: $processId" -ForegroundColor Gray
} else {
    Write-Host "FAILED: TCP Port 8080 is NOT listening" -ForegroundColor Red
...
foreach ($ruleName in $firewallRules) {
    ...
    if ($rule -and $rule.Enabled -eq $true) {
        Write-Host "  SUCCESS: $ruleName - ENABLED" -ForegroundColor Green
    } elseif ($rule -and $rule.Enabled -eq $false) {
        Write-Host "  WARNING: $ruleName - EXISTS but DISABLED" -ForegroundColor Yellow
    } else {
        Write-Host "  FAILED: $ruleName - NOT FOUND" -ForegroundColor Red
    }
}
Write-Host ""
...
foreach ($adapter in $adapters) {
    ...
    Write-Host "  - $ip (Interface: $iface)" -ForegroundColor Cyan
}
Write-Host ""
...
try {
    ...
    # Wait for response
    Write-Host "  Waiting for response (3 seconds timeout)..." -ForegroundColor Gray
    ...
    try {
        ...
        $fromPort = $remoteEP.Port
        Write-Host "SUCCESS: RESPONSE RECEIVED from $fromIp`:$fromPort" -ForegroundColor Green
        Write-Host ""
...
if ($serverProcess) {
    Write-Host "SUCCESS: Found server process(es):" -ForegroundColor Green
    foreach ($proc in $serverProcess) {
        $name = $proc.ProcessName
        $pid = $proc.Id
        Write-Host "  - $name (PID: $pid)" -ForegroundColor Gray
    }
} else {
    Write-Host "FAILED: No server process found" -ForegroundColor Red
...
if (-not $udpConnections) {
    Write-Host "FAILED: UDP Discovery Port NOT listening" -ForegroundColor Red
    $allGood = $false
} else {
    Write-Host "SUCCESS: UDP Discovery Port listening" -ForegroundColor Green
}

if (-not $tcpConnections) {
    Write-Host "FAILED: WebSocket Port NOT listening" -ForegroundColor Red
    $allGood = $false
} else {
    Write-Host "SUCCESS: WebSocket Port listening" -ForegroundColor Green
}
